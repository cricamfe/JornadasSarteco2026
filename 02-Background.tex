...
\begin{tikzpicture}[
    box/.style={draw, thick, minimum height=0.6cm, minimum width=1.0cm, font=\scriptsize\sffamily, rounded corners=2pt},
    sbox/.style={box, fill=cgrared!15},
    mds/.style={box, fill=cgrablue!15},
    rc/.style={box, fill=cgragreen!15},
    arr/.style={-{Stealth[length=3pt]}, thick},
    lbl/.style={font=\footnotesize\sffamily, cgragray},
]
\node[font=\scriptsize\sffamily] (inp) at (0,0) {state};
\node[sbox, right=0.4cm of inp] (sb1) {S-box};
\node[lbl, above=0.02cm of sb1] {$\forall s_i$};
\node[mds, right=0.2cm of sb1] (m1) {MDS};
\node[rc, right=0.2cm of m1] (r1) {$+c_i$};
\draw[arr] (inp) -- (sb1); \draw[arr] (sb1) -- (m1); \draw[arr] (m1) -- (r1);
\node[font=\footnotesize\sffamily\bfseries, above=0.25cm of m1] {Full ($\times R_F/2$)};

\node[sbox, right=0.5cm of r1] (sb2) {S-box};
\node[lbl, above=0.02cm of sb2] {$s_0$};
\node[mds, right=0.2cm of sb2] (m2) {MDS};
\node[rc, right=0.2cm of m2] (r2) {$+c_i$};
\draw[arr] (r1) -- node[above, lbl] {$\cdots$} (sb2); \draw[arr] (sb2) -- (m2); \draw[arr] (m2) -- (r2);
\node[font=\footnotesize\sffamily\bfseries, above=0.25cm of m2] {Partial ($\times R_P$)};

\node[sbox, right=0.5cm of r2] (sb3) {S-box};
\node[lbl, above=0.02cm of sb3] {$\forall s_i$};
\node[mds, right=0.2cm of sb3] (m3) {MDS};
\node[font=\scriptsize\sffamily, right=0.3cm of m3] (out) {out};
\draw[arr] (r2) -- node[above, lbl] {$\cdots$} (sb3); \draw[arr] (sb3) -- (m3); \draw[arr] (m3) -- (out);
\node[font=\footnotesize\sffamily\bfseries, above=0.25cm of m3] {Full ($\times R_F/2$)};
\end{tikzpicture}%
}
\caption{Poseidon2 permutation structure. The S-box ($x^d \bmod p$) dominates cost.}
