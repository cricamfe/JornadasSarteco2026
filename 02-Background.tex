...
Algorithm~\ref{alg:barrett} shows the pseudocode for the S-box layer.

\begin{algorithm}[t]
\caption{S-box $x^7 \bmod p$ on the 4$\times$4 CGRA}\label{alg:barrett}
\begin{algorithmic}[1]
\footnotesize
\Require $x \in [0, p)$, precomputed $\mu = \lfloor 2^{2k}/p \rfloor$
\Ensure $x^7 \bmod p$
\For{$\mathit{op} \in \{\mathrm{sq}_1, \mathrm{mul}_1, \mathrm{sq}_2, \mathrm{mul}_2\}$}
	\State $\mathit{limbs} \gets \Call{ExtractBytes}{\mathit{operands}}$ \Comment{16 PEs parallel}
	\State $\mathit{prod} \gets \Call{MeshMultiply}{\mathit{limbs}}$ \Comment{systolic 4$\times$4}
	\State $\hat{q} \gets \Call{EstimateQuotient}{\mathit{prod}, \mu}$ \Comment{12 PEs + carry}
	\State $r \gets \mathit{prod} - \hat{q} \cdot p$ \Comment{parallel subtract}
	\State $\mathit{result} \gets \Call{Select}{r < p,\; r,\; r - p}$ \Comment{BSFA: 1 cycle}
\EndFor
