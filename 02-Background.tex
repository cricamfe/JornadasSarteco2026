...
\node[sbox, xshift=0.14cm] (sb2) {S-box};
\node[mds]  (m2)  {MDS};
\node[rc]   (r2)  {$+c_i$};

\node[sbox, xshift=0.14cm] (sb3) {S-box};
\node[mds]  (m3)  {MDS};
...
\draw[trans_arr] (r1.east) -- node[above=6pt, lbl] {$\cdots$} (sb2.west);
\draw[arr] (sb2) -- (m2);
\draw[arr] (m2)  -- (r2);

\draw[trans_arr] (r2.east) -- node[above=6pt, lbl] {$\cdots$} (sb3.west);
\draw[arr] (sb3) -- (m3);
\draw[arr] (m3)  -- (out);
