...
\begin{tikzpicture}[
    ...
    internal/.style={box, draw=cgrapurple!70!black, fill=cgrapurple!12},
    rc/.style={box, fill=cgragreen!15},
    arr/.style={-{Stealth[length=2.8pt,width=2.8pt]}, line width=0.60pt, draw=black!80},
    trans_arr/.style={-{Stealth[length=5.2pt,width=5.2pt]}, line width=0.95pt, draw=black!85, shorten <=0pt, shorten >=0pt},
...
\node[on chain, font=\footnotesize\sffamily, inner sep=2pt, xshift=0.06cm] (inp) {state};

\node[rc, xshift=0.10cm]   (r1)  {$+c_i$};
\node[sbox]                (sb1) {S-box};
\node[mds]                 (m1)  {MDS};

\node[rc, xshift=0.14cm]   (r2)  {$+c_i$};
\node[sbox]                (sb2) {S-box};
\node[internal]            (m2)  {MI};

\node[rc, xshift=0.14cm]   (r3)  {$+c_i$};
\node[sbox]                (sb3) {S-box};
\node[mds]                 (m3)  {MDS};

\node[on chain, font=\footnotesize\sffamily, inner sep=2pt, xshift=-0.06cm] (out) {out};
...
\draw[trans_arr] (m1.east) -- node[above=6pt, lbl] {$\cdots$} (r2.west);
\draw[arr] (r2)  -- (sb2);
\draw[arr] (sb2) -- (m2);

\draw[trans_arr] (m2.east) -- node[above=6pt, lbl] {$\cdots$} (r3.west);
\draw[arr] (r3)  -- (sb3);
\draw[arr] (sb3) -- (m3);
\draw[arr] (m3)  -- (out);
...
\node[group_label, above=0.35cm of sb1] {Full ($\times R_F/2$)};

\node[lbl, above=0.05cm of sb2] {$s_0$};
\node[group_label, above=0.35cm of sb2] {Partial ($\times R_P$)};

\node[lbl, above=0.05cm of sb3] {$\forall s_i$};
\node[group_label, above=0.35cm of sb3] {Full ($\times R_F/2$)};
...
\end{figure}
Algorithm~\ref{alg:barrett} shows the pseudocode for the S-box layer, which is based on the modular Barrett multiplication~\cite{barrett1986} (\texttt{MULBARRET()}). We will go through this  code in more detail in  Section~\ref{subsec:sbox-openedge}

\algrenewcommand\algorithmicrequire{\textbf{Input:}}
\algrenewcommand\algorithmicensure{\textbf{Output:}}

\begin{algorithm}
\caption{S-box $x^7 \Mod p$}\label{alg:barrett}
\begin{algorithmic}[1]
\footnotesize
\Require $x \in [0,p)$, and $\mu = \lfloor 2^{2n}/p \rfloor$, $n=\lceil log_2(p-1) \rceil$
\Ensure $x^7 \Mod p$
\State $x^2 \gets \Call{MulBarrett}{x, x}$
\State $x^3 \gets \Call{MulBarrett}{x_2, x}$      
\State $x^4 \gets \Call{MulBarrett}{x_2, x_2}$
\State $x^7 \gets \Call{MulBarrett}{x_4, x_3}$
%\State \Return $x_7$
%\end{algorithmic}

%\begin{algorithmic}[1]
\footnotesize
\Function{MulBarrett}{$a,b$}
    \State $\mathit{prod} \gets \Call{Multiply}{a,b}$ \Comment{Step1: Base Product}
    \State $\hat{q} \gets \Call{QuotientEst}{\mathit{prod}, \mu}$ \Comment{Step2: Quotient Est.}
    \State $r \gets \mathit{prod} - \hat{q}\cdot p$ \Comment{Step3: Mod. Substract.}
    \State \Return $\Call{Select}{r < p,\ r,\ r-p}$ \Comment{Step4: Correction}
\EndFunction
%\\ \Return $x^7$
\end{algorithmic}
...
% \begin{algorithm}[t]
% \caption{S-box $x^7 \Mod p$ on the 4$\times$4 CGRA}\label{alg:barrett}
% \begin{algorithmic}[1]
% {\footnotesize
% \Require $x_1, x_2 \in [0, p)$, precomputed $\mu = \lfloor 2^{2k}/p \rfloor$
% \Ensure $x^7 \bmod p$
% \For{$\mathit{op} \in \{\mathrm{sq}_1, \mathrm{mul}_1, \mathrm{sq}_2, \mathrm{mul}_2\}$}
% %	\State $\mathit{limbs} \gets \Call{ExtractBytes}{\mathit{op}}$ \Comment{Step1: Base Product}
% %	\State $\mathit{prod} \gets \Call{MeshMultiply}{\mathit{limbs}}$ \Comment{16 PEs in parallel}
%     \State $\mathit{prod} \gets \Call{MeshMultiply}{\mathit{op}}$ \Comment{Step1: Base Product}
% 	\State $\hat{q} \gets \Call{EstimateQuotient}{\mathit{prod}, \mu}$ \Comment{Step2: Quot. Est.}
% 	\State $r \gets \mathit{prod} - \hat{q} \cdot p$ \Comment{Step3: Mod. Subst.}
% 	\State $\mathit{result} \gets \Call{Select}{r < p,\; r,\; r - p}$ \Comment{Step4: Correct.}
% \EndFor
...
\begin{tabular}{@{}lllll@{}}
\toprule
\textbf{Work} & \textbf{Platf.} & \textbf{Hash} & \textbf{Field} & \textbf{Reprog.} \\
\midrule
